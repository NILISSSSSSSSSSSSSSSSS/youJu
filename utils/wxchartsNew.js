...
function drawPointShape(points, color, shape, context,opts, shapeStyle) {
    context.beginPath();
    ...
    do{
        if(shapeStyle) {
             points.forEach(function (item, index) {
                if (item !== null) {
                    let width = shapeStyle.width;
                    let height = shapeStyle.height;
                    context.moveTo(item.x + 3.5*opts.pixelRatio, item.y);
                    let pointX = item.x - width/2;
                    if(index == points.length - 1) {
                        pointX = item.x - width/2 - 4;
                    }
                    context.drawImage(shapeStyle.img, pointX, item.y - height/2, width, height);
                }
            });
            context.closePath();
            context.fill();
        ...
        if (shape === 'diamond') {
            ...
            points.forEach(function (item, index) {
                if (item !== null) {
                    context.moveTo(item.x + 3.5*opts.pixelRatio, item.y);
                    context.arc(item.x, item.y, 8*opts.pixelRatio, 0, 2 * Math.PI, false);
                }
            });
        } else if (shape === 'rect') {
            points.forEach(function (item, index) {
...
function drawToolTipSplitLine(offsetX, opts, config, context) {
    ...
    context.beginPath();
    context.setStrokeStyle(opts.tooltip && opts.tooltip.option && opts.tooltip.option.split 
        ? opts.tooltip.option.split.color || "#cccccc" : "#cccccc");
    context.setLineWidth(opts.tooltip && opts.tooltip.option && opts.tooltip.option.split 
        ? opts.tooltip.option.split.lineWidth || 1*opts.pixelRatio : 1*opts.pixelRatio);
...
function drawToolTip(textList, offset, opts, config, context) {
    ...
    textList.forEach(function (item, index) {
		...
		opts.tooltip.option.showLegend = opts.tooltip.option.showLegend === undefined 
			? true : opts.tooltip.option.showLegend
		if(opts.tooltip.option && opts.tooltip.option.showLegend) {
    ...
    textList.forEach(function (item, index) {
		...
		context.beginPath();
		context.setFontSize(config.fontSize);
		context.setFillStyle(opts.tooltip && opts.tooltip.option 
            ? opts.tooltip.option.color || '#ffffff' : '#ffffff');
        context.fillText(item.text, startX, startY + config.fontSize - 2);
		context.closePath();
		context.stroke();
    });
}
...
function drawAreaDataPoints(series, opts, config, context) {
    ...
    series.forEach(function (eachSeries, seriesIndex) {
        ...
        splitPointList.forEach(function (points) {
            ...
            do{
                // 渐变色
                if(opts.backgroudStyle && opts.backgroudStyle.show) {
                    points.forEach(function (item, index) {
                        if(points[index+ 1] && (index % 2 == 0)) {
                            if(opts.backgroudStyle.img) {
                                context.drawImage(opts.backgroudStyle.img, item.x, 10,
                                points[index+ 1].x - item.x, endY - 8);
                            }else{
                                const grd = context.createLinearGradient(item.x, 10, points[index+ 1].x , endY - 8);
                                grd.addColorStop(0, opts.backgroudStyle.startColor || '#fefdf8')
                                grd.addColorStop(1, opts.backgroudStyle.endColor || '#fffbe8')
                                context.setFillStyle(grd);
                                context.fillRect(item.x, 10, points[index+ 1].x - item.x, endY - 8);
                            }
                        }
                    });
                    break;
			...
			context.beginPath();
			context.setStrokeStyle(eachSeries.color);
			context.setLineWidth(eachSeries.lineWidth || 2*opts.pixelRatio);
			if (points.length === 1) {
        ...
        if (opts.dataPointShape !== false) {
            ...
            drawPointShape(points, eachSeries.color, shape, context,opts, eachSeries.shapeStyle);
        }
...
function drawXAxis(categories, opts, config, context) {
    ...
    if (config._xAxisTextAngle_ === 0) {
        ...
        categories.forEach(function (item, index) {
			...
			context.beginPath();
			context.setFontSize(config.fontSize);
			context.setFillStyle(opts.xAxis.fontColor || '#a3a5a8');
            context.fillText(item, xAxisPoints[index] + offset - 10, startY + config.fontSize + 5);
			context.closePath();
			context.stroke();
        });
...
function drawYAxisGrid(categories,opts, config, context) {
    ...
    // context.setLineWidth(1*opts.pixelRatio);
    // 使用自己定义的宽度
    context.setLineWidth(opts.yAxis.lineWidth || 1*opts.pixelRatio);
...
function drawYAxis(series, opts, config, context) {
    ...
    rangesFormat.forEach(function (item, index) {
		...
		context.beginPath();
		context.setFontSize(config.fontSize);
		context.setFillStyle(opts.yAxis.fontColor || '#a3a5a8');
...
function drawCharts(type, opts, config, context, callback = function () {}) {
    var _this = this;
...
var Charts = function Charts(opts, callback = function () {}) {
    ...
    drawCharts.call(this, opts.type, opts, config$$1, this.context, function (type) {
        callback(type);
    });
};
    ...
    if (this.opts.type === 'line' || this.opts.type === 'area') {
        ...
        if (index > -1) {
            ...
            if (seriesData.length !== 0) {
                ...
                opts.tooltip["textList"] = textList;
                opts.tooltip["offset"] = offset;
                opts.tooltip["option"] = Object.assign(option, (opts.tooltip["option"] || {}));
                //不能重置对象
                // opts.tooltip = {
                //     textList: textList,
                //     offset: offset,
