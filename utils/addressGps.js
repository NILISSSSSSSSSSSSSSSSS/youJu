...
function setLocalAdcode(adcode, city,lat,lng) {
  uni.setStorage({
    key: adcodeKey,
    data: {lng:lng,lat:lat,adcode: adcode, city: city, timestamp: (new Date()).valueOf()}
  })
}
...
export function getAdcodeHere(onSuccess, onFailed=function () {}) {
  ...
  wxLocation()
    ...
    .then((regeocode)=>{
      ...
      setLocalAdcode(adcode, city,lat,lng);
