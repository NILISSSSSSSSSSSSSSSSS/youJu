<template>
	<view>
		...
		<view class="slider_view">
            <view :style="{left: movePositionLeft > 84 ? 84 + '%' : movePositionLeft + '%',transform: 'translateX(' + (-((movePositionLeft > 84 ? 84 : movePositionLeft)/1.28)/2) + 'px)'}" 
...
<script>
	...
	export default {
		...
		methods: {
			...
			upAreaSlider() {
				this.sliderVal = this.currentVal;
				this.sliderAreaMoveChange({detail: {value: this.transSliderHouseAreaVal()}});
			},
			// 更新初始化slider价格
			upPriceSlider() {
				this.sliderVal = this.currentVal;
			...
			sliderAreaMoveChange(e) {
				...
				sliderMoveChangeTimeOutVal = setTimeout(() => {
                    ...
                    this.sliderVal = val;
					this.$emit("onSliderChanged", {areaMin: areaL, areaMax: areaH, type: this.type, areaText: areaMoveChoose,
                        moveChooseVal: areaMoveChoose});
