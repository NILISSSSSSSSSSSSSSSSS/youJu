...
<template>
    <view :class="{ active: isActive }" class="er_house_list_cont"
        ...
        <view @click="searchAnimationClick" class="top_view" :animation="searchAnimation">
            <search-btn-view :caseType="'1'" :name="searchBuildName"></search-btn-view>
            <screen-house :city="siteMapParams.cityId" :regId="siteMapParams.regId"
                :enterType="'erHouse'"></screen-house>
        </view>
		...
		<no-data-trust :type="'erHouse'" v-if="erHouseList && erHouseList.length === 0"></no-data-trust>
        ...
        <view class="er_house_list_view" v-else>
            ...
            <block v-for="(item, index) in erHouseList" :key="index">
				<new-house-list-item v-if="item.caseType == 9 || item.caseType == 6" :item="item"></new-house-list-item>
...
<script>
    ...
    import {GetErHouseList} from "../../net/GetErHouseList";
	...
	export default {
        ...
        methods: {
            ...
            upGpsInfo() {
                ...
                new this.SetMetaInfo().initErHouseList();
            },
        	...
        	getErHouseList() {
                return new Promise((resolve, reject) => {
                    ...
                    new GetErHouseList(this.dealParams(params)).send()
                       .then(res => {
                           let data = res.DATA || {};
                           let list = data.list || [];
                           if(params.pageNum === 1) {
            ...
            drawErHouseList(requestData = {}) {
                ...
                for (let i = 0;i<apiList.length;i++) {
                    ...
                    // 现在又换成java的接口了 ，以前是php的
                    // java接口用这个处理插入的新房
                    if(dataItem.caseType && (dataItem.caseType == 9 || dataItem.caseType == 6)
						&& this.drawNewHouseItem(dataItem)) {
					...
					// 现在又换成java的接口了 ，以前是php的
                    // php接口用这个方法处理插入的新房
                    // if(i !== 0 && (i+1)%6 === 0 && newHouseList[parseInt((i+1)/6)]) {
                    //     erHouseList.push(this.drawNewHouseItem(newHouseList[parseInt((i+1)/6)]));
                    // }
                }
                return erHouseList;
            },
