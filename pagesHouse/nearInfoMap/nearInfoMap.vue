<template>
	<view :style="{height: pageHeight}" class="near_info_map">
		...
		<view class="map_cont f_c_s">
			<map-btn-view class="map_btn" :currentIndex="currentIndex"></map-btn-view>
...
<script>
	...
	import mapBtnView from "@/pagesHouse/components/erHouseDetail/mapBtnView.vue";
		...
		methods: {
			dealParams(params) {
				...
				uni.setNavigationBarTitle({
					title: params.name + (this.keyword ? "周边" + this.keyword : "")
				});
				this.upMapData();
			},
			...
			upMapData() {
				...
				new SetMap().searchNearInfo(this.keyword, this.adcode, this.longitude, this.latitude)
					.then(tips => {
