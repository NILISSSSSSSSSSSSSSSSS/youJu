<template>
	<form report-submit @submit="formSubmit">
		<view class="traffic_info_view">
			<view class="traffic_info" :class="{ traffic_info_hidden : !isLookMore }">{{ item.busLine+'奥术大师大所大所多' }}</view>

			<view v-if="isShowLookBtn" @click="lookMore" class="more_btn f_r_c">
...
<script>
	export default {
		...
		methods: {
			...
			initBtnShow() {
				setTimeout(() => {
						this.DifferenceApi.boundingClientRect(".traffic_info", this).then(data => {
								if(!data || !data.height) {
