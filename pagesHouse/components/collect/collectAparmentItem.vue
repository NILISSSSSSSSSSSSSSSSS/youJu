<template>
	<view @click="checkApartmentDetail(item)" class="near_er_house_item f_c_s">
		...
		<view class="near_er_house_cont">
			<view class="near_er_house_title">{{ item.houseAddress }} {{ item.houseName }} {{ item.houseTing }} {{ item.houseJianText }}</view>
			
			<view v-if="item.houseTags && item.houseTags.length > 0" class="collect_er_house_tag_info f_r_s">
				<block v-for="(tagInfo, index) in item.houseTags" :key="index">
					<view class="collect_er_house_tag f_r_c">{{ tagInfo }}</view>
				</block>
			</view>
			
			<view class="near_er_house_price_view f_r_s" v-if="item.housePriceOfMonth">
