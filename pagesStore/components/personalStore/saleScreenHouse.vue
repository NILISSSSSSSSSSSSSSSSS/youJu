...
<script>
	...
	export default {
        ...
        methods: {
            ...
            // 初始化列表
            initData() {
				if(!this.agentCityId) return Promise.resolve({});

			    return new GetScreenList({cityId: this.agentCityId}).send()
				   .then(res => {
						let data = res.DATA;
            ...
            getParams() {
                ...
                for(let key in data) {
                    ...
                    if(item.rightId != undefined && (item.leftId !== item.rightId || !item.rightId)) {
						...
						params["sectionName"] = item.rightId ? data.region.text : "";
                    }
                }
                return params;
            },
