			...
			<view class="screen_view">
				...
				<view class="screen_list f_r_b" v-if="enterType == 'erHouse'">
					<view hover-class="none" form-type="submit" @click="screenBtn('region')" class="screen_item f_r_c">
						<view :style="{color: screenFormData.erHouse.region.show || screenFormData.erHouse.region.text != '区域' ? '#2491FF' : '#494949'}"
					...
					<view hover-class="none" form-type="submit" @click="screenBtn('price')" class="screen_item f_r_c">
						<view :style="{color: screenFormData.erHouse.price.show || screenFormData.erHouse.price.text != priceApiDataMap[from].defaultText ? '#2491FF' : '#494949'}"
					...
					<view hover-class="none" form-type="submit" @click="screenBtn('room')" class="screen_item f_r_c">
						<view :style="{color: screenFormData.erHouse.room.show || screenFormData.erHouse.room.text != '户型' ? '#2491FF' : '#494949'}"
					...
					<view hover-class="none" form-type="submit" @click="screenBtn('more')" class="screen_item f_r_c">
						<view :style="{color: screenFormData.erHouse.more.show || screenFormData.erHouse.more.text != '更多' ? '#2491FF' : '#494949'}"
				...
				<view class="screen_list f_r_b" v-if="enterType == 'newHouse'">
					<view hover-class="none" form-type="submit"
						...
						<view :style="{color: screenFormData[enterType].region.show || screenFormData[enterType].region.text != '区域' ? '#2491FF' : '#494949'}"
					...
					<view hover-class="none" form-type="submit" @click="screenBtn('price')" class="screen_item f_r_c">
						<view :style="{color: screenFormData.newHouse.price.show || screenFormData.newHouse.price.text != priceApiDataMap[from].defaultText ? '#2491FF' : '#494949'}"
					...
					<view hover-class="none" form-type="submit" @click="screenBtn('type')" class="screen_item f_r_c">
						<view :style="{color: screenFormData.newHouse.type.show || screenFormData.newHouse.type.text != '用途' ? '#2491FF' : '#494949'}"
				...
				<view class="screen_list f_r_b" v-if="enterType == 'apartment'">
					<view hover-class="none" form-type="submit" @click="screenBtn('region')" class="screen_item f_r_c">
						<view :style="{color: screenFormData.apartment.region.show || screenFormData.apartment.region.text != '区域' ? '#2491FF' : '#494949'}"
					...
					<view hover-class="none" form-type="submit" @click="screenBtn('price')" class="screen_item f_r_c">
						<view :style="{color: screenFormData.apartment.price.show || screenFormData.apartment.price.text != priceApiDataMap[from].defaultText ? '#2491FF' : '#494949'}"
					...
					<view hover-class="none" form-type="submit" @click="screenBtn('chuZuType')" class="screen_item f_r_c">
						<view :style="{color: screenFormData.apartment.chuZuType.show || screenFormData.apartment.chuZuType.text != '出租方式' ? '#2491FF' : '#494949'}"
					...
					<view hover-class="none" form-type="submit" @click="screenBtn('more')" class="screen_item f_r_c">
						<view :style="{color: screenFormData.apartment.more.show || screenFormData.apartment.more.text != '更多' ? '#2491FF' : '#494949'}"
				...
				<view @click="screenClose" :style="{top: fixedTcTop}" class="screen_fixed_list" v-if="listTcShow">
					<!-- 区域 -->
					...
					<view :style="{height: '55%'}" @click.stop="screenContBtn"
						...
						<scroll-view class="scroll_view_list price_scroll_list"
							...
							<block v-for="(item, index) in newHouseTypeList" :key="index">
								<view hover-class="none" form-type="submit"
									v-if="item.text.indexOf('地产') < 0"
									:class="{screen_active: newHouseTypeListIndex == index}"
									@click="newHouseTypeBtn(item, index)"
									class="region_list_item">{{ item.text }}</view>
							</block>
						</scroll-view>
					</view>
...
<script>
    ...
    import { GetNewScreenList, GetScreenList } from "@/net/GetScreenList.js";
	...
	export default {
        ...
        methods: {
            ...
            setTabNodeInfo() {
                setTimeout(() => {
                    let view = this.DifferenceApi.createSelectorQuery(this).select(".screen_list");
                    view.boundingClientRect(data => {
						...
						// #endif
                        if(data && data.height) {
                            this.fixedTcTop = data.height + data.top + h5TitleHeight + 1 + "px";
                        }else{
							// #ifdef MP-ALIPAY
            ...
            upGpsInfo() {
                ...
                this.initData().then(() => {
					if(new GpsInfoModel().getModel("cityInfo") && !new SearchHouseModel().getModel()
            ...
            // 初始化列表
            initData() {
				if(!this.agentCityId || !this.archiveId || !this.compId) return Promise.resolve({});

				return Promise.all([
					new GetScreenList({cityId: this.agentCityId}).send(),
					new GetNewScreenList({
						brokerCityId: this.agentCityId,
						archiveId: this.archiveId,
						compId: this.compId,
						personalVersion: 1}).send()
					]).then(info => {
						let res = info[0];
						let data = res.DATA;
						let cityList = info[1].data ? info[1].data.cityList || [] : [];
						this.drawRegionList(cityList);
            ...
            // 初始化区域
            drawRegionList(apiData = []) {
                let regionLeftList = [{id: "", text: "不限"}];
				let regionRightMap = {"": [{id: "", text: "不限"}]};
                for(let i = 0;i<apiData.length;i++) {
                    let dataItem = apiData[i] || {};
                    if(!dataItem.cityId || !dataItem.cityName) continue;

                    regionLeftList.push({
                       id: dataItem.cityId,
                       text: dataItem.cityName
                    });
					let rightList = dataItem.regList || [];
					let list = [{id: "", text: "不限"}];
					for(let j = 0;j<rightList.length;j++) {
					    let infoItem = rightList[j] || {};
					    if(!infoItem.regionId || !infoItem.regionName) continue;

					    list.push({
					       id: infoItem.regionId,
					       text: infoItem.regionName
					    });
					}
					regionRightMap[dataItem.cityId] = list;
                }
            ...
            getParams() {
                ...
                for(let key in data) {
                    ...
                    if(item.rightId != undefined) {
                        params[item["key_2"]] = item.rightId;
						params["regionName"] = item.rightId ? data.region.text : "";
                    }
                }
                return params;
            },
            ...
            regionLeftBtn(item, index, needNc = true) {
                ...
                if(!regionRightMap[item.id] || regionRightMap[item.id].length <= 1) {
                    ...
                    screenFormData[enterType].region.text = (!item.id ? "区域" : item.text);
                    if(!item.id) {
                        screenFormData[enterType].region.rightId = "";
                    }
                    this.listTcShow = false;
...
<style>
   ...
   /* .screen_item:not(:last-child)::before{
    ...
    .more_btn_view .confirmBtn{
        background:-webkit-linear-gradient(left, #2491FF , #2491FF);
		color:#fff;
    }
	...
	.price_bottom_confirm{
		width:200upx;
		height:74upx;
		background-image:linear-gradient(246deg, #2491FF 0%, #2491FF 100%), linear-gradient( #2491FF, #2491FF);
		border-radius:37upx;
		color:#ffffff;
		font-size:28upx;
		line-height:74upx;
